YUI.add('bpp-jsonp', function(Y){
    ...
    */
    ...
    var cache = new Y.CacheOffline({
        sandbox:'bpp-jsonp',
        expires: Y.bpp.config('misc.cache') || (24*2*60*60*1000),    // 48 hours. could be longer, no?
    ...
    Y.bpp.jsonp = function(url, c, options){
        url = url+'';
    ...
    // expose cache
    Y.bpp.jsonp.cache = cache;

},'@VERSION@',{requires:['jsonp','jsonp-url','cache-offline', 'bpp-config']});