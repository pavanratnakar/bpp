YUI.add('bpp-template', function(Y){
    // template helpers
    var $ = {
        translate: function(){}
    };
	...
	var template = function (str) {
        ...
        return (function(options){
            ...
            options.assets = options.assets || Y.bpp.util.assets;
            options.translate = options.translate || $.translate;
            options.sanitize = options.sanitize || Y.bpp.util.sanitize;
            return fn(options);
        });
    };
    Y.template = template;
},'@VERSION@', {requires:['base', 'bpp-util']});